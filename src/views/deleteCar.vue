...
export default {
    ...
    data() {
        return {
            tableColumns: ['licensePlate', 'price', 'startTime', 'endTime', 'note'], // 表格標題
            maintenanceData: [], // 表格內容
            searchText: '', // 搜尋關鍵字
    ...
    mounted() { // 預設執行方法，進入頁面後隨即執行，後端-顯示所有站點資料
        fetch("http://localhost:8080/find_all_finished_abnormal")
            ...
            .then(data => {
              console.log(data);
              this.maintenanceData = data.maintenanceList})
    },
    methods: { // 各種方法
        updateFilteredData() { // 更新數據內容的方法
            ...
            return this.maintenanceData.filter(item => // 將資料進行過濾，回傳含關鍵字資料
